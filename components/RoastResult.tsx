              ...
              {sections.map(({ key, data }, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className="rounded-lg sm:rounded-xl border border-gray-200 dark:border-white/[0.06] bg-gray-50/50 dark:bg-white/[0.02] overflow-hidden"
                  >
                    <button
                      onClick={() => setExpandedSection(isExpanded ? null : key)}
                      className="w-full px-3 sm:px-4 py-2.5 sm:py-3.5 flex items-center gap-2.5 sm:gap-3 text-left hover:bg-gray-100/50 dark:hover:bg-white/[0.02] transition-colors"
                    >
                      {/* Step number indicator */}
                      <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-gray-200 dark:bg-white/10 flex items-center justify-center flex-shrink-0">
                        <span className="text-[10px] sm:text-xs font-medium text-gray-600 dark:text-gray-400">
                          {index + 1}
                        </span>
                      </div>

                      <span className="font-medium text-sm sm:text-base text-gray-900 dark:text-white flex-1">
                            ...
                            <div className="pl-7 sm:pl-9 space-y-3 sm:space-y-4">
                              ...
                              {data.insights && data.insights.length > 0 && (
                                <div className="space-y-1.5 sm:space-y-2">
                                  <p className="text-[10px] sm:text-xs font-medium text-gray-500 dark:text-gray-500 uppercase tracking-wider">
                                    Key Insights
                                  </p>
                                  <ul className="space-y-1.5 sm:space-y-2">
                                    {data.insights.map((insight, i) => (
                                      ...
                                      >
                                        <span className="w-1 h-1 rounded-full bg-orange-500 mt-1.5 sm:mt-2 flex-shrink-0" />
                                        <span>{insight}</span>
                                      </motion.li>
                                    ))}
                                  </ul>
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="mt-10 sm:mt-16 pt-6 sm:pt-8 border-t border-gray-100 dark:border-gray-800 w-full flex flex-col items-center"
      >
        <div className="flex flex-col items-center gap-4 sm:gap-6 w-full max-w-sm px-2">
          <div className="grid grid-cols-1 w-full">
            <motion.button
              onClick={onReset}
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              className="flex items-center justify-center px-4 py-2.5 border border-gray-200 dark:border-gray-800 text-xs sm:text-sm font-medium rounded-lg text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors active:scale-95"
            >
