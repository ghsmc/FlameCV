...
export const RoastResult: React.FC<RoastResultProps> = ({ 
  ...
  const FIX_BUTTON_FLAG = "[[SHOW_FIX_BUTTON]]";

  const { cleanContent, showFixButton } = useMemo(() => {
    const hasFlag = content.includes(FIX_BUTTON_FLAG);
    const cleaned = content.replace(FIX_BUTTON_FLAG, '').trim();
    const shouldShow = hasFlag && !!onFix && !isFixMode;
    return { cleanContent: cleaned, showFixButton: shouldShow };
  }, [content, onFix, isFixMode]);
